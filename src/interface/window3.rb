...
class BounceWindow < FXMainWindow
    ...
    def initialize(app)
        ...
        @ball = Ball.new(20)
        @ppu_image = FXImage.new(app, nil, width: 256, height: 240)
    end
    ...
    def drawScene(drawable)
        ...
        pixels = Array.new(256 * 240, FXRGB(0, 255, 0))
        @backBuffer.setPixels(pixels, 0, @width, @height)
        # dc = FXDCWindow.new(@ppu_image)
        # dc.drawImage(@ppu_image, 0, 0)
        #         dc.end
        FXDCWindow.new(drawable) { |dc|
            # dc = FXDCWindow.new(@ppu_image)
